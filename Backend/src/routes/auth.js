...
router.get('/google/callback', async (req, res) => {
  ...
  try {
    ...
    await prisma.recruiter.update({
      ...
      data: {
        googleRefreshToken: tokens.refresh_token || prisma.recruiter.fields.googleRefreshToken, // Preserve if not returned
        googleAccessToken: tokens.access_token,
        googleTokenExpiry: tokenExpiry,
        googleEmail: googleEmail, // Optional: store connected Google account
        calendarConnectedAt: new Date() // Timestamp for analytics
      }
    });
